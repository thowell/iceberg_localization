...
class body():
    def __init__(self, n=20, r=60, var=3, x=0, y=0, vx=-0.1, vy=-0.1, omega=-0.01):
        ...
        self.feature_dim = 2
        self.polygon = self.gen_polygon(self.n, self.r,
                                        self.var)  # generate 2D polygon representation of body in polar coordinates [r,theta]
...
class simulator():
    ...
    def run_dynamics(self):
        ...
        self.offset_desired = np.mean(
            [self.model.min_standoff, self.model.max_standoff])  # desired radius to circle body
        ...
        for k in range(1, n_steps):
            ...
            if self.mode == 1:
                u = self.model.control(self.model.v, mode=self.mode, error=error[k - 1], error_prev=error[k - 2],
                                       error_array=error[self.K:])
            ...
            if self.mode == 0:
                error[k] = self.r_desired - np.sqrt(
                    X_no_measurement[0, k] ** 2 + X_no_measurement[1, k] ** 2)  # idealized
            elif self.mode == 1:
                Y = np.zeros((self.body.feature_dim, self.body.n))
                for j in range(self.body.n):
                    Y[:, j] = self.model.measurement(X[:, k - 1:k], Map[:, j:j + 1]).ravel()
    ...
    def run_filter(self, filter, results, initialization_mode=0):
        ...
        for k in range(1, results['n_steps']):
            ...
            if k < results['K']:
                ...
                if k == results['K']:
                    print('---Arrived at body---')
                    MU[:, :, k - 1] = filter.particle_initialization(results['X_no_measurement'][:, k:k + 1],
                                                                     filter.sigma0,
                                                                     mode=initialization_mode)  # initialize particle filter using current odometry
                ...
                Y = np.zeros((self.body.feature_dim, self.body.n))
                for j in range(self.body.n):
                    Y[:, j] = self.model.measurement(results['X'][:, k:k + 1], results['map'][:, j:j + 1, k]).ravel()
                idx = Y[0, :].ravel().argsort()[
                      :self.model.measurement_limit]  # only update using the closest measurements
                MU[:, :, k] = filter.measurement_update(mu_, Y, idx, results['T'][k],
                                                        self.tf)  # measurement update for particles
...
if __name__ == "__main__":
    ...
    N_particles = [10, 100, 1000, 10000]  # number of particles
